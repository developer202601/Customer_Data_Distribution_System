...
<tr>
    ...
    <td class="text-end">
        <div class="d-flex justify-content-end gap-2">
            <a href="{{ route('cc.super.edit_region', $a) }}" class="btn btn-sm btn-outline-secondary rounded-pill">Edit</a>
            @if($a->status)
            ...
            @endif
            @if(!$a->fixed)
                <button type="button" class="btn btn-sm btn-outline-danger rounded-pill cc-delete-btn" data-action="{{ route('cc.users.destroy', $a) }}" data-username="{{ $a->username }}">Delete</button>
            @endif
        </div>
    </td>
</tr>
@empty
<tr><td colspan="5" class="text-muted">No region admins yet.</td></tr>
@endforelse