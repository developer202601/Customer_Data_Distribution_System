                    ...
                    <div class="card border-0 bg-light rounded-4 mb-4">
                        <div class="card-body d-flex justify-content-between align-items-start flex-wrap gap-3">
                            <div>
                                ...
                                <p class="mb-0 small text-muted">Enable this to use this page and require regional review
                                    before distribution.</p>
                                @if(!empty($reviewOptIn) && !empty($reviewEnabledAt))
                                    <p class="mb-0 small text-muted">Only reports generated on/after
                                        {{ $reviewEnabledAt->format('Y-m-d H:i') }} can be reviewed here.</p>
                                @endif
                            </div>
                            <form method="post" action="{{ route('cc.region.review.preference') }}"
                                class="d-flex align-items-center gap-2 ms-auto">
                                @csrf
                                <input type="hidden" name="enable_regional_review"
                                    value="{{ !empty($reviewOptIn) ? '0' : '1' }}">
                                <span
                                    class="badge {{ !empty($reviewOptIn) ? 'text-bg-success' : 'text-bg-secondary' }}">{{ !empty($reviewOptIn) ? 'Enabled' : 'Disabled' }}</span>
                                <button type="submit"
                                    class="btn btn-sm {{ !empty($reviewOptIn) ? 'btn-outline-danger' : 'btn-outline-success' }} rounded-pill px-3">
                    ...
                    <div id="reviewWorkspace" class="{{ empty($reviewOptIn) ? 'review-workspace-disabled' : '' }}">
                        ...
                        <form method="get" id="reviewFiltersForm" class="row g-2 align-items-end mb-3" data-loader-off="1">
                            ...
                            <input type="hidden" id="show_hidden" name="show_hidden"
                                value="{{ !empty($showHidden) ? '1' : '0' }}">
                            <input type="hidden" id="show_hidden_only" name="show_hidden_only"
                                value="{{ !empty($showHiddenOnly) ? '1' : '0' }}">
                        </form>
                        ...
                        @if($selectedReport)
                                ...
                                <div class="col-md-4">
                                    <div class="card border-0 bg-light rounded-4 h-100">
                                        <div class="card-body">
                                            ...
                                            <h2 class="h5 mb-0" id="countTotalRows">{{ number_format($counts['total'] ?? 0) }}
                                            </h2>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="card border-0 bg-light rounded-4 h-100">
                                        <div class="card-body">
                                            ...
                                            <h2 class="h5 mb-0" id="countVisibleRows">
                                                {{ number_format($counts['visible'] ?? 0) }}</h2>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="card border-0 bg-light rounded-4 h-100">
                                        <div class="card-body">
                                            ...
                                            <h2 class="h5 mb-0" id="countHiddenRows">{{ number_format($counts['hidden'] ?? 0) }}
                                            </h2>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="card border-0 bg-light rounded-4">
                                    <div class="card-body p-3">
                                        ...
                                        <form method="post" action="#" enctype="multipart/form-data"
                                            ...
                                            @csrf
                                            <input type="file" name="attach_file" id="attachFileInput"
                                                class="form-control form-control-sm"
                                                accept=".pdf,.doc,.docx,.xls,.xlsx,.txt,.csv" />
                                            <button type="submit" class="btn btn-primary btn-sm px-3"
                                                style="white-space: nowrap;">Attach File</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="d-flex flex-wrap gap-2 mb-3 align-items-center">
                                ...
                                @if(!empty($reviewRecord?->reviewed_at))
                                    <span class="small text-success">Passed at
                                        {{ $reviewRecord->reviewed_at->format('Y-m-d H:i') }}</span>
                                @else
                                    ...
                                    <div class="card-body p-3">
                                        ...
                                        <form method="post" action="#" enctype="multipart/form-data"
                                            ...
                                            <button type="submit" class="btn btn-success btn-sm px-2 py-1"
                                                style="white-space: nowrap;">Submit Exclude File</button>
                                        </form>
                                    </div>
                                </div>
                            </div>

                            <div id="bulkActionsDock" class="bulk-actions-dock mb-3"
                                data-locked="{{ !empty($reviewRecord?->reviewed_at) ? '1' : '0' }}">
                                <div class="bulk-actions-card" id="bulkActionsCard">
                                    <div class="small text-muted mb-1" id="bulkActionsSelectionHint">Select rows to manage
                                        visibility.</div>
                                    <div class="small text-warning-emphasis mb-2 d-none" id="bulkActionsMixedHint">Both visible
                                        and hidden rows are selected. Choose the correct action below.</div>
                                    <div class="d-flex flex-wrap align-items-center gap-2">
                                        <button type="button" class="btn btn-outline-danger btn-sm bulk-action-btn d-none"
                                            data-action="hide" id="bulkHideBtn">Hide Selected Rows</button>
                                        <button type="button" class="btn btn-outline-secondary btn-sm bulk-action-btn d-none"
                                            data-action="unhide" id="bulkUnhideBtn">Unhide Selected Rows</button>
                                        <button type="button" class="btn btn-outline-primary btn-sm d-none"
                                            id="bulkClearSelectionBtn">Clear selection</button>
                                        <span class="small text-muted d-none" id="bulkActionsCountBadge"></span>
                                    </div>
                                </div>
                            </div>

                            <form id="bulkRowsForm" method="post"
                                action="{{ route('cc.region.review.hide_rows', $selectedReport->id) }}">
                                @csrf
                            ...
                            <div class="modal fade" id="bulkActionConfirmModal" tabindex="-1"
                                aria-labelledby="bulkActionConfirmTitle" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-scrollable align-content-center">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="bulkActionConfirmTitle">Confirm row visibility update
                                            </h5>
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <button type="button" class="btn btn-primary btn-sm"
                                                id="bulkActionConfirmSubmit">Confirm</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        @else
                            @if(!empty($reviewOptIn) && !empty($reviewEnabledAt))
                                <div class="alert alert-info mb-0">No reviewable reports found for your region after
                                    {{ $reviewEnabledAt->format('Y-m-d H:i') }}.</div>
                            @else
